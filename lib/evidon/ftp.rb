...
module Evidon
  ...
  module FTP
    ...
    def self.prepare_staging_js
      ...
      ret = js.gsub!( %r{^\s*DOMAIN_ROOT = \(\('https:' == document\.location\.protocol\) \? 'https://c\.betrad\.com' : 'http://c\.betrad\.com'\),\s*$}, '  DOMAIN_ROOT = ( ("https:" == document.location.protocol) ? "https://" : "http://"  ) + "staginginfo.evidon.com/ftp/staging",')
      if ret.nil? then
    ...
    def self.update_surly(cap_env, repository, scm_username = nil, scm_password = nil)
      repository.chomp!("/")
      @surly_repo = "#{repository}/surly/surly.js"
      ...
      if cap_env == "staging"
        js.gsub!(/_surly_url = \(\("https:" == document\.location\.protocol\) \? "https:\/\/c\.betrad\.com" : "http:\/\/c\.betrad\.com"\);/, "_surly_url = ( ('https:' == document.location.protocol) ? 'https://' : 'http://' ) + 'staginginfo.evidon.com/ftp/staging';")
        js.gsub!(/\/geo\/ba\.js/, "/ba.min.js")
        js.gsub!(/if \(_surly_src\.match\('surly.js'\) \&\& _surly_own\) \{/, "if (_surly_src.match('surly.js') && (_surly_src.indexOf('staginginfo.evidon') > -1 && _surly_src.indexOf('staginginfo.evidon') < 50) ) {")
      end
